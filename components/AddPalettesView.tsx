...
interface AddPalettesViewProps {
  onClose: () => void;
...
const AddPalettesView: React.FC<AddPalettesViewProps> = ({ onClose, onAdd, palettes, folderName }) => {
  ...
  return (
    <aside
      className="lg:h-[calc(100vh-4rem)] bg-white rounded-2xl shadow-sm border border-gray-200/80 flex flex-col"
      role="complementary"
      aria-labelledby="add-palettes-view-title"
    >
      <header className="p-4 border-b border-gray-200 flex justify-between items-center flex-shrink-0">
        <h2 id="add-palettes-view-title" className="text-lg font-bold text-gray-800 truncate pr-2">Add to "{folderName}"</h2>
      ...
      <main className="p-4 flex-1 overflow-y-auto">
        {palettes.length > 0 ? (
          <div className="grid grid-cols-1 gap-3">
             <p className="text-sm text-gray-500 mb-2">Select palettes to drag into the folder, or use the button at the bottom.</p>
            {palettes.map((item) => {
...
export default AddPalettesView;
